...
#NOTE: -c(37) removes the unlogged SalePrice
df.clean.allcor <- cor(df.clean.numeric[,-c(37)], use="pairwise.complete.obs")
...
#NOTE: -c(34) removes the unlogged SalePrice
df.clean.allcor <- cor(df.clean.numeric[,-c(34)], use="pairwise.complete.obs")
#The cutoff point for correlation, currently randomly assigned
corr_amt <- 0.6
#Gets a list of all correlations with higher than 'corr_amt' of correlation
df.clean.highcor <- rmCorDup(subset(as.data.frame(as.table(df.clean.allcor)), (abs(Freq) > corr_amt) & (abs(Freq) < 1)))
...
#Fit for Forward, Stepwide, Backward - 
#REMOVED HeatingQC, one of the factors only has 1 level
df.clean.train.fit <- lm(L_SalePrice ~ MSSubClass + MSZoning + LotFrontage + LotArea + Street + LotShape + LandContour +LotConfig + LandSlope + Neighborhood + Condition1 + Condition2 + BldgType + HouseStyle + OverallQual + OverallCond + YearBuilt + YearRemodAdd + RoofStyle + RoofMatl + Exterior1st + Exterior2nd + MasVnrType + MasVnrArea + ExterQual + ExterCond + Foundation + BsmtQual + BsmtCond + BsmtExposure + BsmtFinType1 + BsmtFinSF1 + BsmtFinType2 + BsmtFinSF2 + BsmtUnfSF + TotalBsmtSF + Heating + CentralAir + Electrical + X1stFlrSF + X2ndFlrSF + LowQualFinSF + GrLivArea + BsmtFullBath + BsmtHalfBath + FullBath + HalfBath + BedroomAbvGr + KitchenAbvGr + KitchenQual + Functional + Fireplaces + FireplaceQu + GarageType + GarageFinish + GarageCars + GarageQual + GarageCond + PavedDrive + WoodDeckSF + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + PoolArea + MiscVal + MoSold + YrSold + SaleType + SaleCondition, data = df.clean.train)

#Lasso
#df.clean.lasso <- glmnet(df.clean.train[,c(1:72)], df.clean.train$L_SalePrice, family="gaussian", alpha=1)
...
df.clean.train.step.vif <- makevif(df.clean.train.step$model[,-c(1)], df.clean.test$L_SalePrice)
...
#Summary of forward with p-values
summary(df.clean.train.forw)
#The VIF's for the data (only numeric), the -c(1) removes the y component (L_SalePrice)
df.clean.train.forw.vif <- makevif(df.clean.train.forw$model[,-c(1)], df.clean.test$L_SalePrice)
df.clean.train.forw.vif
#Overall AIC for the model
df.clean.train.forw.aic <- df.clean.train.forw$anova$AIC[ length(df.clean.train.forw$anova$AIC) ]
#Plots for the model
mlrplots(df.clean.train.forw)

#Backward------------------------------------------
df.clean.train.back <- stepAIC(df.clean.train.fit, direction="backward", trace=FALSE)
#Summary of backward with p-values
summary(df.clean.train.back)
#The VIF's for the data (only numeric), the -c(1) removes the y component (L_SalePrice)
df.clean.train.back.vif <- makevif(df.clean.train.back$model[,-c(1)], df.clean.test$L_SalePrice)
df.clean.train.back.vif
#Overall AIC for the model
df.clean.train.back.aic <- df.clean.train.back$anova$AIC[ length(df.clean.train.back$anova$AIC) ]
#Plots for the model
mlrplots(df.clean.train.back)

#Custom

#Lasso

#Stepwise RMSE, have to do this otherwise errors---------------
...
#Forward RMSE, have to do this otherwise errors---------------
...
#Backward RMSE, have to do this otherwise errors---------------
...
#TEMOPORARY CUSTOM MODEL----------------------------
#df.clean.train.cust <- lm(L_SalePrice ~ OverallQual + TotalBsmtSF + GrLivArea + GarageCars, data = df.clean.train)
...
#rmse(df.clean.pred.cust, df.clean.test$SalePrice)
