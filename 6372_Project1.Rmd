...
# Tej added this section below: 
library(corrplot)
#Gets only the numeric values for the scatterplots
df.clean.numeric <- df.clean[, sapply(df.clean, is.numeric)]
cat('There are', length(df.clean.numeric), 'numeric variables')
#correlations of all numeric variables
corr_numVar <- cor(df.clean.numeric, use="pairwise.complete.obs")
#sort on decreasing correlations with SalePrice
corr_sorted <- as.matrix(sort(corr_numVar[,'SalePrice'], decreasing = TRUE))
#select only high corelations
corrHigh <- names(which(apply(corr_sorted, 1, function(x) abs(x)>0.5)))
corr_numVar <- corr_numVar[corrHigh, corrHigh]

corrplot.mixed(corr_numVar, tl.col="black", tl.pos = "lt")
```
